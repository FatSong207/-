...
namespace Yuebon.Chaochi.Repositories
{
    ...
    /// 
    /// </summary>
    public class BuildingRepository : BaseRepository<Building, string>, IBuildingRepository
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        public BuildingRepository(IDbContextCore dbContext) : base(dbContext)
        {
        }
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<BuildingPropoties>> GetAllBuildingPropotiesAsync()
        {
            var datalist = await DapperConn.QueryAsync<BuildingPropoties>("select * from Chaochi_BuildingPropoties order by Id");
        ...
        public void DoubleUpdate(BuildingEquipment buildingEquipment, BuildingEq buildingEq)
        {
            using (IDbConnection connection = DapperConn) {
                ...
                using (var tran = connection.BeginTransaction()) {
                    connection.Insert(buildingEquipment,tran);
                    connection.Insert(buildingEq,tran);
        ...
        /// <returns></returns>
        public async Task<Building> GetByBAdd(string BAdd)
        {
            string sql = @"SELECT * FROM Chaochi_Building t WHERE t.BAdd = @BAdd";
            return await DapperConn.QueryFirstOrDefaultAsync<Building>(sql, new { BAdd = BAdd });
        }
        ...
        /// <returns></returns>
        public async Task<Building> GetByBID(string BID)
        {
            string sql = @"SELECT * FROM Chaochi_Building t WHERE t.BID = @BID";
            return await DapperConn.QueryFirstOrDefaultAsync<Building>(sql, new { BID = BID });
        }
        ...
        public bool Insert(Building entity, Security.Models.UserLogOn userLogOnEntity, IDbTransaction trans = null)
        {
            ...
            DbContext.GetDbSet<Building>().Add(entity);
