...
namespace Yuebon.Chaochi.Repositories
{
    ...
    /// 
    /// </summary>
    public class CustomerLNRepository : BaseRepository<CustomerLN, string>, ICustomerLNRepository
    {
        /// <summary>
        /// 
        /// </summary>
        public CustomerLNRepository()
        {
        }

        public CustomerLNRepository(IDbContextCore dbContext) : base(dbContext)
        {
        }
        ...
        {
            string sql = @"SELECT * FROM Chaochi_CustomerLN t WHERE t.LNID = @LNID";
            if (conn != null) {
                return await conn.QueryFirstOrDefaultAsync<CustomerLN>(sql, new { @LNID = LNID }, transaction: tran);
            } else {
                return await DapperConn.QueryFirstOrDefaultAsync<CustomerLN>(sql, new { @LNID = LNID }, transaction: tran);
            }
        }
