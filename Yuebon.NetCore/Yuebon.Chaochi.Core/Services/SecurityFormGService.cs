...
namespace Yuebon.Chaochi.Services
{
    ...
    /// 
    /// </summary>
    public class SecurityFormGService : BaseService<SecurityFormG, SecurityFormGOutputDto, string>, ISecurityFormGService
    {
        private readonly ISecurityFormGRepository _repository;
        ...
        public SecurityFormGService(ISecurityFormGRepository repository) : base(repository)
        {
        ...
        public async Task<FileInfoOutputDto> GetImgOutDtoAsync(string id)
        {
            ...
            string uploadPath = @$"{path}BuildingImage\{id}\SecurityFormG\";
            if (System.IO.Directory.Exists(uploadPath)) {
                ...
                List<MyFileInfo> myfiles = dbfiles.Select(x => new MyFileInfo {Id=x.Id, FileName = x.fileName,seq=x.seq,title=x.title,size=x.size,isAppear=x.isAppear }).OrderBy(p=>Convert.ToInt32(p.seq)).ToList();
