...
namespace Yuebon.Chaochi.Services
{
    ...
    /// 
    /// </summary>
    public class ExternalformService : BaseService<Externalform, ExternalformOutputDto, string>, IExternalformService
    {
        private readonly IExternalformRepository _userRepository;
        ...
        /// <returns></returns>
        public async Task<Externalform> GetByFormId(string FormId)
        {
            return await _userRepository.GetByFormId(FormId);
        }
        ...
        public bool Insert(Externalform entity, Security.Models.UserLogOn userLogOnEntity, IDbTransaction trans = null)
        {
        ...
        /// <param name="userLogOnEntity"></param>
        /// <param name="trans"></param>
        public async Task<bool> InsertAsync(Externalform entity, List<BuildingBelonging> buildingBelongingEntitys, IDbTransaction trans = null)
        {
        ...
        /// <param name="userLogOnEntity"></param>
        /// <param name="trans"></param>
        public bool Insert(Externalform entity, Security.Models.UserLogOn userLogOnEntity, Security.Models.UserOpenIds userOpenIds, IDbTransaction trans = null)
        {
        ...
        //    CustomerLN user = userInPut.MapTo<User>();
        ...
        public async Task<PageResult<ExternalformOutputDto>> FindWithPagerSearchAsync(SearchExternalformModel search)
        {
            ...
            if (!string.IsNullOrEmpty(search.FormId)) {
                where += string.Format(" and FormId like '%{0}%'", search.FormId);
            }
            if (!string.IsNullOrEmpty(search.FormName)) {
                where += string.Format(" and FormName like '%{0}%'", search.FormName);
            }
            //if (!string.IsNullOrEmpty(search.GuildCountyCity)) {
            //    where += string.Format(" and GuildCountyCity like '%{0}%'", search.GuildCountyCity);
            //}
            if (!string.IsNullOrEmpty(search.CustTag))
            {
                where += string.Format(" and CustTag like '%{0}%'", search.CustTag);
            }
            //if (!string.IsNullOrEmpty(search.Type))
            //{
            //    where += string.Format(" and Type like '%{0}%'", search.Type);
            //}
            //if (!string.IsNullOrEmpty(search.Period)) {
            //    where += string.Format(" and Period like '%{0}%'", search.Period);
            //}
            if (!string.IsNullOrEmpty(search.selectedCategoryOptions))
            {
                where += string.Format(" and level like '{0}%'", search.selectedCategoryOptions);
            }
            ...
            List<Externalform> list = await repository.FindWithPagerAsync(where, pagerInfo, search.Sort, order);
            List<ExternalformOutputDto> resultList = list.MapTo<ExternalformOutputDto>();
            /*
            ...
            */
            PageResult<ExternalformOutputDto> pageResult = new PageResult<ExternalformOutputDto>
            {
        ...
        /// <returns></returns>
        public override async Task<ExternalformOutputDto> GetOutDtoAsync(string id)
        {
            Externalform info = await repository.GetAsync(id);
            ExternalformOutputDto ExternalformOutputDto = info.MapTo<ExternalformOutputDto>();
            ...
            //ExternalformOutputDto.BuildingBelongings = BuildingBelongings;
            ...
            //ExternalformOutputDto.BuildingImages = myfiles;
            */
            return ExternalformOutputDto;
        }
