    ...
    public class IpAddressUtil
    {
        ...
        public static string GetCityByIp(string strIP)
        {
            ...
            try
            {
                ...
                {
                    string nation = ipResult.result.ad_info.nation.ToString();//國家
                    string province = ipResult.result.ad_info.province.ToString();//省份
                    string city = ipResult.result.ad_info.city.ToString();//城市
                    string district = ipResult.result.ad_info.district.ToString();//區/縣
                    string adcode = ipResult.result.ad_info.adcode.ToString();//行政區劃代碼
                    string resultStr = "";
                ...
                {
                    Log4NetHelper.Error(strIP + "獲取地區接口調用異常。" + ipResult.message);
                    return "未知";
                }
            }
            catch(Exception ex)
            {
                Log4NetHelper.Error(strIP+"獲取地區異常。"+ex.Message);
                return "未知";
            }
            */
        }
