tinymce.addI18n('zh_CN',{
"Redo": "恢復",
"Undo": "撤銷",
"Cut": "剪切",
"Copy": "復制",
"Paste": "粘貼",
"Select all": "全選",
"New document": "新建文檔",
"Ok": "確定",
"Cancel": "取消",
"Visual aids": "網格線",
"Bold": "粗體",
"Italic": "斜體",
"Underline": "下劃線",
"Strikethrough": "刪除線",
"Superscript": "上標",
"Subscript": "下標",
"Clear formatting": "清除格式",
"Align left": "左對齊",
"Align center": "居中",
"Align right": "右對齊",
"Justify": "兩端對齊",
"Bullet list": "符號列表",
"Numbered list": "數字列表",
"Decrease indent": "減少縮進",
"Increase indent": "增加縮進",
"Close": "關閉",
"Formats": "格式",
"Your browser doesn't support direct access to the clipboard. Please use the Ctrl+X\/C\/V keyboard shortcuts instead.": "當前瀏覽器不支持訪問剪貼板，請使用快捷鍵Ctrl+X/C/V復制粘貼",
"Headers": "標題",
"Header 1": "標題1",
"Header 2": "標題2",
"Header 3": "標題3",
"Header 4": "標題4",
"Header 5": "標題5",
"Header 6": "標題6",
"Headings": "標題",
"Heading 1": "標題1",
"Heading 2": "標題2",
"Heading 3": "標題3",
"Heading 4": "標題4",
"Heading 5": "標題5",
"Heading 6": "標題6",
"Preformatted": "預格式化",
"Div": "Div區塊",
"Pre": "預格式文本",
"Code": "代碼",
"Paragraph": "段落",
"Blockquote": "引用",
"Inline": "文本",
"Blocks": "區塊",
"Paste is now in plain text mode. Contents will now be pasted as plain text until you toggle this option off.": "當前為純文本粘貼模式，再次點擊可以回到普通粘貼模式。",
"Fonts": "字體",
"Font Sizes": "字號",
"Class": "Class",
"Browse for an image": "瀏覽圖像",
"OR": "或",
"Drop an image here": "拖放一張圖片文件至此",
"Upload": "上傳",
"Block": "塊",
"Align": "對齊",
"Default": "默認",
"Circle": "空心圓",
"Disc": "實心圓",
"Square": "方塊",
"Lower Alpha": "小寫英文字母",
"Lower Greek": "小寫希臘字母",
"Lower Roman": "小寫羅馬字母",
"Upper Alpha": "大寫英文字母",
"Upper Roman": "大寫羅馬字母",
"Anchor...": "錨點...",
"Name": "名稱",
"Id": "id",
"Id should start with a letter, followed only by letters, numbers, dashes, dots, colons or underscores.": "id應該以字母開頭，后跟字母、數字、橫線、點、冒號或下劃線。",
"You have unsaved changes are you sure you want to navigate away?": "你對文檔的修改尚未保存，確定離開嗎？",
"Restore last draft": "恢復上次的草稿",
"Special characters...": "特殊字符...",
"Source code": "HTML源碼",
"Insert\/Edit code sample": "插入/編輯代碼示例",
"Language": "語言",
"Code sample...": "代碼示例...",
"Color Picker": "選取顏色",
"R": "R",
"G": "G",
"B": "B",
"Left to right": "從左到右",
"Right to left": "從右到左",
"Emoticons...": "表情符號...",
"Metadata and Document Properties": "元數據和文檔屬性",
"Title": "標題",
"Keywords": "關鍵詞",
"Description": "描述",
"Robots": "機器人",
"Author": "作者",
"Encoding": "編碼",
"Fullscreen": "全屏",
"Action": "操作",
"Shortcut": "快捷鍵",
"Help": "幫助",
"Address": "地址",
"Focus to menubar": "移動焦點到菜單欄",
"Focus to toolbar": "移動焦點到工具欄",
"Focus to element path": "移動焦點到元素路徑",
"Focus to contextual toolbar": "移動焦點到上下文菜單",
"Insert link (if link plugin activated)": "插入鏈接 (如果鏈接插件已激活)",
"Save (if save plugin activated)": "保存(如果保存插件已激活)",
"Find (if searchreplace plugin activated)": "查找(如果查找替換插件已激活)",
"Plugins installed ({0}):": "已安裝插件 ({0})：",
"Premium plugins:": "優秀插件：",
"Learn more...": "了解更多...",
"You are using {0}": "你正在使用 {0}",
"Plugins": "插件",
"Handy Shortcuts": "快捷鍵",
"Horizontal line": "水平分割線",
"Insert\/edit image": "插入/編輯圖片",
"Image description": "圖片描述",
"Source": "地址",
"Dimensions": "大小",
"Constrain proportions": "保持寬高比",
"General": "常規",
"Advanced": "高級",
"Style": "樣式",
"Vertical space": "垂直邊距",
"Horizontal space": "水平邊距",
"Border": "邊框",
"Insert image": "插入圖片",
"Image...": "圖片...",
"Image list": "圖片列表",
"Rotate counterclockwise": "逆時針旋轉",
"Rotate clockwise": "順時針旋轉",
"Flip vertically": "垂直翻轉",
"Flip horizontally": "水平翻轉",
"Edit image": "編輯圖片",
"Image options": "圖片選項",
"Zoom in": "放大",
"Zoom out": "縮小",
"Crop": "裁剪",
"Resize": "調整大小",
"Orientation": "方向",
"Brightness": "亮度",
"Sharpen": "銳化",
"Contrast": "對比度",
"Color levels": "色階",
"Gamma": "伽馬值",
"Invert": "反轉",
"Apply": "應用",
"Back": "后退",
"Insert date\/time": "插入日期/時間",
"Date\/time": "日期/時間",
"Insert\/Edit Link": "插入/編輯鏈接",
"Insert\/edit link": "插入/編輯鏈接",
"Text to display": "顯示文字",
"Url": "地址",
"Open link in...": "鏈接打開方式...",
"Current window": "當前窗口打開",
"None": "在當前窗口/框架打開",
"New window": "在新窗口打開",
"Remove link": "刪除鏈接",
"Anchors": "錨點",
"Link...": "鏈接...",
"Paste or type a link": "粘貼或輸入鏈接",
"The URL you entered seems to be an email address. Do you want to add the required mailto: prefix?": "你所填寫的URL地址為郵件地址，需要加上mailto:前綴嗎？",
"The URL you entered seems to be an external link. Do you want to add the required http:\/\/ prefix?": "你所填寫的URL地址屬于外部鏈接，需要加上http://:前綴嗎？",
"Link list": "鏈接列表",
"Insert video": "插入視頻",
"Insert\/edit video": "插入/編輯視頻",
"Insert\/edit media": "插入/編輯媒體",
"Alternative source": "替代資源",
"Alternative image URL": "資源備用地址",
"Media poster (Image URL)": "封面(圖片地址)",
"Paste your embed code below:": "將內嵌代碼粘貼在下面:",
"Embed": "內嵌",
"Media...": "多媒體...",
"Nonbreaking space": "不間斷空格",
"Page break": "分頁符",
"Paste as text": "粘貼為文本",
"Preview": "預覽",
"Print...": "打印...",
"Save": "保存",
"Find": "查找",
"Replace with": "替換為",
"Replace": "替換",
"Replace all": "替換全部",
"Previous": "上一個",
"Next": "下一個",
"Find and replace...": "查找并替換...",
"Could not find the specified string.": "未找到搜索內容。",
"Match case": "區分大小寫",
"Find whole words only": "全單詞匹配",
"Spell check": "拼寫檢查",
"Ignore": "忽略",
"Ignore all": "忽略全部",
"Finish": "完成",
"Add to Dictionary": "添加到字典",
"Insert table": "插入表格",
"Table properties": "表格屬性",
"Delete table": "刪除表格",
"Cell": "單元格",
"Row": "行",
"Column": "列",
"Cell properties": "單元格屬性",
"Merge cells": "合并單元格",
"Split cell": "拆分單元格",
"Insert row before": "在上方插入",
"Insert row after": "在下方插入",
"Delete row": "刪除行",
"Row properties": "行屬性",
"Cut row": "剪切行",
"Copy row": "復制行",
"Paste row before": "粘貼到上方",
"Paste row after": "粘貼到下方",
"Insert column before": "在左側插入",
"Insert column after": "在右側插入",
"Delete column": "刪除列",
"Cols": "列",
"Rows": "行",
"Width": "寬",
"Height": "高",
"Cell spacing": "單元格外間距",
"Cell padding": "單元格內邊距",
"Show caption": "顯示標題",
"Left": "左對齊",
"Center": "居中",
"Right": "右對齊",
"Cell type": "單元格類型",
"Scope": "范圍",
"Alignment": "對齊方式",
"H Align": "水平對齊",
"V Align": "垂直對齊",
"Top": "頂部對齊",
"Middle": "垂直居中",
"Bottom": "底部對齊",
"Header cell": "表頭單元格",
"Row group": "行組",
"Column group": "列組",
"Row type": "行類型",
"Header": "表頭",
"Body": "表體",
"Footer": "表尾",
"Border color": "邊框顏色",
"Insert template...": "插入模板...",
"Templates": "模板",
"Template": "模板",
"Text color": "文字顏色",
"Background color": "背景色",
"Custom...": "自定義...",
"Custom color": "自定義顏色",
"No color": "無",
"Remove color": "刪除顏色",
"Table of Contents": "目錄",
"Show blocks": "顯示區塊邊框",
"Show invisible characters": "顯示不可見字符",
"Word count": "字數統計",
"Words: {0}": "字數：{0}",
"{0} words": "{0} 個字",
"File": "文件",
"Edit": "編輯",
"Insert": "插入",
"View": "查看",
"Format": "格式",
"Table": "表格",
"Tools": "工具",
"Powered by {0}": "Powered by {0}",
"Rich Text Area. Press ALT-F9 for menu. Press ALT-F10 for toolbar. Press ALT-0 for help": "在編輯區按ALT+F9打開菜單，按ALT+F10打開工具欄，按ALT+0查看幫助",
"Image title": "圖片標題",
"Border width": "邊框寬度",
"Border style": "邊框樣式",
"Error": "錯誤",
"Warn": "警告",
"Valid": "有效",
"To open the popup, press Shift+Enter": "此快捷為軟回車（插入<br>）",
"Rich Text Area. Press ALT-0 for help.": "編輯區. 按Alt+0鍵打開幫助",
"System Font": "默認字體",
"Failed to upload image: {0}": "圖片上傳失敗: {0}",
"Failed to load plugin: {0} from url {1}": "插件加載失敗: {0} - {1}",
"Failed to load plugin url: {0}": "插件加載失敗: {0}",
"Failed to initialize plugin: {0}": "插件初始化失敗: {0}",
"example": "示例",
"Search": "查找",
"All": "全部",
"Currency": "貨幣",
"Text": "文本",
"Quotations": "引用",
"Mathematical": "數學運算符",
"Extended Latin": "拉丁語擴充",
"Symbols": "符號",
"Arrows": "箭頭",
"User Defined": "自定義",
"dollar sign": "美元",
"currency sign": "貨幣",
"euro-currency sign": "歐元",
"colon sign": "冒號",
"cruzeiro sign": "克魯賽羅幣",
"french franc sign": "法郎",
"lira sign": "里拉",
"mill sign": "密爾",
"naira sign": "奈拉",
"peseta sign": "比塞塔",
"rupee sign": "盧比",
"won sign": "韓元",
"new sheqel sign": "新謝克爾",
"dong sign": "越南盾",
"kip sign": "老撾基普",
"tugrik sign": "圖格里克",
"drachma sign": "德拉克馬",
"german penny symbol": "德國便士",
"peso sign": "比索",
"guarani sign": "瓜拉尼",
"austral sign": "澳元",
"hryvnia sign": "格里夫尼亞",
"cedi sign": "塞地",
"livre tournois sign": "里弗弗爾",
"spesmilo sign": "一千spesoj的貨幣符號，該貨幣未使用",
"tenge sign": "堅戈",
"indian rupee sign": "印度盧比",
"turkish lira sign": "土耳其里拉",
"nordic mark sign": "北歐馬克",
"manat sign": "馬納特",
"ruble sign": "盧布",
"yen character": "日元",
"yuan character": "人民幣元",
"yuan character, in hong kong and taiwan": "元的繁體字",
"yen\/yuan character variant one": "元（大寫）",
"Loading emoticons...": "正在加載表情文字...",
"Could not load emoticons": "不能加載表情文字",
"People": "人類",
"Animals and Nature": "動物和自然",
"Food and Drink": "食物和飲品",
"Activity": "活動",
"Travel and Places": "旅游和地點",
"Objects": "物件",
"Flags": "旗幟",
"Characters": "字數",
"Characters (no spaces)": "字數（不含空格）",
"Error: Form submit field collision.": "錯誤: 表單提交字段沖突.",
"Error: No form element found.": "錯誤: 未找到可用的form.",
"Update": "更新",
"Color swatch": "顏色樣本",
"Turquoise": "青綠",
"Green": "綠色",
"Blue": "藍色",
"Purple": "紫色",
"Navy Blue": "海軍藍",
"Dark Turquoise": "深藍綠色",
"Dark Green": "暗綠",
"Medium Blue": "中藍",
"Medium Purple": "中紫",
"Midnight Blue": "深藍",
"Yellow": "黃色",
"Orange": "橙色",
"Red": "紅色",
"Light Gray": "淺灰",
"Gray": "灰色",
"Dark Yellow": "暗黃",
"Dark Orange": "暗橙",
"Dark Red": "暗紅",
"Medium Gray": "中灰",
"Dark Gray": "深灰",
"Black": "黑色",
"White": "白色",
"Switch to or from fullscreen mode": "切換全屏模式",
"Open help dialog": "打開幫助對話框",
"history": "歷史",
"styles": "樣式",
"formatting": "格式化",
"alignment": "對齊",
"indentation": "縮進",
"permanent pen": "記號筆",
"comments": "注釋",
"Anchor": "錨點",
"Special character": "特殊字符",
"Code sample": "代碼示例",
"Color": "顏色",
"Emoticons": "表情",
"Document properties": "文檔屬性",
"Image": "圖片",
"Insert link": "插入鏈接",
"Target": "目標",
"Link": "鏈接",
"Poster": "封面",
"Media": "音視頻",
"Print": "打印",
"Prev": "上一個",
"Find and replace": "查找并替換",
"Whole words": "全字匹配",
"Spellcheck": "拼寫檢查",
"Caption": "標題",
"Insert template": "插入模板",
//以下為補充漢化內容 by 莫若卿
"Code view": "代碼區域",
"Select...": "選擇...",
"Format Painter": "格式刷",
"No templates defined.": "無內置模板",
"Special character...": "特殊字符...",
"Open link": "打開鏈接",
"None": "無",
"Count": "統計",
"Document": "整個文檔",
"Selection": "選取部分",
"Words": "字詞數",
"{0} characters": "{0} 個字符",
"Alternative source URL": "替代資源地址",
"Alternative description": "替代說明文字",
"Accessibility": "可訪問性",
"Image is decorative": "僅用于裝飾",
//5.6新增
"Line height": "行高",
"Cut column": "剪切列",
"Copy column": "復制列",
"Paste column before": "粘貼到前方",
"Paste column after": "粘貼到后方",
"Copy column": "復制列",
//幫助窗口內的文字
"Version": "版本",
"Keyboard Navigation": "鍵盤導航",
"Open popup menu for split buttons": "該組合鍵的作用是軟回車（插入br）",
});