...
import { GetPersonalTodoListCount } from '@/api/chaochi/todolist/todolistservice'
...
const mutations = {
  ...
  SET_TODOLISTCOUNT: (state, todolistCount) => {
    state.todolistCount = todolistCount
  },
...
const actions = {
  userlogin({ commit }, loginInfo) {
    ...
    return new Promise((resolve, reject) => {
      login({ username: username.trim(), password: password, vcode: vcode, vkey: verifyCodeKey, appId: loginInfo.appId, systemCode: loginInfo.systemCode }).then(response => {
    ...
    return new Promise((resolve, reject) => {
      getUserInfo().then(response => {
        if (response.Success) {
          ...
          commit('SET_TODOLISTCOUNT', data.TodoListCount)
          commit('SET_USERID', data.UserId)
          commit('SET_Dept', data.DeptId)
  ...
  settodoListCount({ commit }) {
    return new Promise(resolve => {
      GetPersonalTodoListCount(state.name).then(res => {
        commit('SET_TODOLISTCOUNT', res.ResData)
        resolve()
      })
    })
  },
