<template>
  <el-form-item label="" prop="LNAdd_1">
    <el-select v-model="county" clearable style="width: 120px" placeholder="縣市">
      <el-option v-for="item in countries" :key="item.Id" :label="item.value" :value="item.Id" />
    </el-select>
    {{ " " }}
    <el-select v-model="town" clearable style="width: 120px" placeholder="鄉鎮市區">
      <el-option v-for="item in towns2" :key="item.Index" :label="item.site_id" :value="item.site_id" />
    </el-select>
    {{ " " }}
    <el-select
      ref="select"
    ...
    >
      <el-option v-for="item in streets" :key="item.Index" :label="item.road" :value="item.road" />
    </el-select>
    {{ " " }}
    <el-input v-model="addressExceptCountyTown" style="width:200px" />
  </el-form-item>
</template>
...
export default {
  ...
  methods: {
    ...
    sendAddress() {
      ...
      instance.Add_4 = this.addressExceptCountyTown
    ...
    async handleAddressExceptCountyTown() {
      this.addressExceptCountyTown = this.sendedform.Add_4
    },
    remoteMethod(query) {
      if (query !== '') {
        this.loading = true
