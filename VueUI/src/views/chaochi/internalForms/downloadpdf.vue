...
import {
  ...
  downloadWebFormHistoryFormById
...
import { saveCustomerRC } from '@/api/chaochi/customerrc/customerrc';
...
export default {
            ...
            if (this.formid === 'A000001') {
              this.$refs['searchform'].validate(valid => {
                if (valid) {
                  ...
                  CheckCustomerLNExists(this.searchPDF.LNID)
                    .then(res => {
                      if (res.ResData !== 'deny') {
                        ...
                        if (this.checkResult) {
                          ...
                          GetRemittancesByIDNo(this.searchPDF.LNID).then(
                            res => {
          ...
          if (this.searchPDF.LCID.length === 8) {
            if (this.formid === 'A000003') {
              this.$refs['searchform'].validate(valid => {
                if (valid) {
                  this.sureBtnLoading = true;
                  CheckCustomerLNExists(this.searchPDF.LCID)
                    .then(res => {
                      if (res.ResData !== 'deny') {
                        this.checkResult = !res.ResData;
                        if (this.checkResult) {
                          ...
                          GetRemittancesByIDNo(this.searchPDF.LCID).then(
                            res => {
                              this.LRemittanceArr = res.ResData;
                              this.sureBtnDisable = false;
                            }
          ...
          if (this.searchPDF.RNID.length === 10) {
            if (this.formid === 'A000002') {
              this.$refs['searchform'].validate(valid => {
                if (valid) {
                  this.sureBtnLoading = true;
                  CheckCustomerRNExists(this.searchPDF.RNID)
                    .then(res => {
                      if (res.ResData !== 'deny') {
                        ...
                        if (this.checkResult2) {
                          ...
                          GetRemittancesByIDNoR(this.searchPDF.RNID).then(
                            res => {
    ...
    'searchPDF.RCID': {
      handler() {
        ...
        if (this.searchPDF.RCID.length > 0) {
          if (this.searchPDF.RCID.length === 8) {
            if (this.formid === 'A000004') {
              this.$refs['searchform'].validate(valid => {
                if (valid) {
                  this.sureBtnLoading = true;
                  CheckCustomerRNExists(this.searchPDF.RCID)
                    .then(res => {
                      if (res.ResData !== 'deny') {
                        this.checkResult2 = !res.ResData;
                        if (this.checkResult2) {
                          ...
                          GetRemittancesByIDNoR(this.searchPDF.RCID).then(
                            res => {
                              this.RRemittanceArr = res.ResData;
                              this.sureBtnDisable = false;
                            }
  ...
  methods: {
    ...
    InserCustomerR() {
      this.$prompt('請輸入承租人姓名', '提示', {
        ...
        if (this.searchPDF.RNID) {
          ...
          this.loading = true;
          saveCustomerRC(data, 'CustomerRC/Insert')
            .then(res => {
              if (res.Success) {
                this.$message.success('新增成功');
                this.checkResult2 = false;
    ...
    PushRoute() {
      ...
      this.loading = true;
      this.searchPDF.ROUserName = this.realname;
      var disabled =
        (this.searchPDF.LRemittanceId !== 'new' &&
          this.searchPDF.LRemittanceId !== '') ||
        (this.searchPDF.RRemittanceId !== 'new' &&
          this.searchPDF.RRemittanceId !== '');
