...
export default {
  ...
  data() {
    return {
      httpFileUploadUrl: defaultSettings.apiChaochiUrl + 'CustomerLC/ImgUpload',
      historytableData: [],
  ...
  methods: {
      ...
      if (this.currentSelected.length === 0) {
        ...
        this.$confirm('是否確認刪除所選的數據項?', '警告', {
          ...
          .then(() => {
            ...
            return DeleteLCHistoryForm(data);
          })
          .then((res) => {
            if (res.Success) {
              this.$message({
    ...
    async downloadformAsync() {
      if (
        ...
        await GetLCFileNameById(this.currentId).then((res) => {
          this.filename = res.ResData;
          var reg = new RegExp('^([0-9]{14})');
          this.filename = this.filename.replace(reg, '');
        });
        downLoadLCForm(this.currentId, this.id).then((res) => {
    ...
    doSearch() {
      ...
      getLCHistoryFormListWithPager(seachHistoryFormdata).then((res) => {
