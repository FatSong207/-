  ...
  >
    ...
    >
        ...
        <el-form-item label="出租人匯款資訊：">
          ...
          >
            ...
            <el-option
              ...
              :key="item.Id"
              :label="
                item.LAName + ' _ ' + (item.LAID === null ? '' : item.LAID)
              "
              :value="item.Id"
            >
      ...
      <el-row>
        <el-form-item>
          <el-checkbox
            v-model="searchPDF.ReplaceROFieldCheck"
          >以下列指定資訊，取代此表單欄位內容</el-checkbox>
        </el-form-item>
      </el-row>
  ...
  methods: {
    ...
    CreateBindBuilding() {
      ...
      var belongId = this.searchPDF.LNID
        ? this.searchPDF.LNID
        : this.searchPDF.LCID;
      InsertFromWebForm(data, belongId, 'Building/InsertFromWebForm')
        .then((res) => {
          if (res.Success) {
            this.$message.success('新增成功');
