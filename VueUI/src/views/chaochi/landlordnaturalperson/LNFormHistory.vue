...
export default {
  ...
  data() {
    return {
      httpFileUploadUrl: defaultSettings.apiChaochiUrl + 'CustomerLN/ImgUpload',
      historytableData: [],
  ...
  methods: {
      ...
      if (this.currentSelected.length === 0) {
        ...
        this.$confirm('是否確認刪除所選的數據項?', '警告', {
          ...
          .then(() => {
            ...
            return DeleteLNHistoryForm(data);
          })
          .then((res) => {
            if (res.Success) {
              this.$message({
    ...
    async downloadformAsync() {
      if (
        ...
        await GetLNFileNameById(this.currentId).then((res) => {
          this.filename = res.ResData;
          var reg = new RegExp('^([0-9]{14})');
          this.filename = this.filename.replace(reg, '');
        });
        downLoadLNForm(this.currentId, this.id).then((res) => {
    ...
    doSearch() {
      ...
      getLNHistoryFormListWithPager(seachHistoryFormdata)
        .then((res) => {
    ...
    loadTableData: function() {
      ...
      getLNHistoryFormListWithPager(seachHistoryFormdata).then((res) => {
        this.historytableData = res.ResData.Items;
        this.pagination.pageTotal = res.ResData.TotalItems;
        this.tableloading = false;
      });
