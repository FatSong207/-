  ...
  >
    ...
    <el-form-item label=" ">
      <font
        color="red"
      >新增上傳:
        負責人身分證、存摺、營利事業登記證明、變更事項登記卡、代理人身分證、授權書...等
      </font>
    </el-form-item>
    <el-form-item label=" ">
      <el-button-group>
...
export default {
  ...
  data() {
    return {
      httpFileUploadUrl: defaultSettings.apiChaochiUrl + 'CustomerRC/ImgUpload',
      historytableData: [],
  ...
  methods: {
      ...
      if (this.currentSelected.length === 0) {
        ...
        this.$confirm('是否確認刪除所選的數據項?', '警告', {
          ...
          .then(() => {
            ...
            return DeleteRCHistoryForm(data);
          })
          .then((res) => {
            if (res.Success) {
              this.$message({
    ...
    async downloadformAsync() {
      if (
        ...
        await GetRCFileNameById(this.currentId).then((res) => {
          this.filename = res.ResData;
          var reg = new RegExp('^([0-9]{14})');
          this.filename = this.filename.replace(reg, '');
        });
        downLoadRCForm(this.currentId, this.id).then((res) => {
    ...
    doSearch() {
      ...
      getRCHistoryFormListWithPager(seachHistoryFormdata).then((res) => {
