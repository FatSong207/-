<template>
  ...
  >
    ...
    <el-form-item label="">
      <font color="red">
        新增上傳:
        第一類建物謄本、房屋稅單、稅籍證明、建築物使用執照、測量成果圖...等,建物相關附件
      </font>
    </el-form-item>
    <el-form-item label="">
      <el-button-group>
    ...
    >
      <el-form inline>
        ...
        <el-form-item label="">
          <font
            color="red"
          >*請註意您只能上傳PDF、JPG、JPEG、PNG、GIF、<br>RAR、ZIP</font>
        </el-form-item>
      </el-form>
    </el-dialog>
  </el-form>
...
<script>
import {
  getBuildingHistoryFormListWithPager,
  downLoadBuildingForm,
  GetBuildingFileNameById,
  DeleteBuildingHistoryForm
} from '@/api/chaochi/historyform/historyformservice';
import defaultSettings from '@/settings';
import { getToken } from '@/utils/auth';
export default {
  name: 'BuildingFormHistory',
  props: {
  ...
  data() {
    return {
      // httpFileUploadUrl: defaultSettings.apiChaochiUrl + 'Building/ImgUpload',
      httpFileUploadUrl:
  ...
  methods: {
      ...
      if (this.currentSelected.length === 0) {
        ...
        this.$confirm('是否確認刪除所選的數據項?', '警告', {
          ...
          .then(() => {
            ...
            return DeleteBuildingHistoryForm(data);
          })
          .then((res) => {
            if (res.Success) {
              this.$message({
    ...
    async downloadformAsync() {
      if (
        ...
        await GetBuildingFileNameById(this.currentId).then((res) => {
          this.filename = res.ResData;
          var reg = new RegExp('^([0-9]{14})');
          this.filename = this.filename.replace(reg, '');
        });
        downLoadBuildingForm(this.currentId, this.id).then((res) => {
    ...
    doSearch() {
      ...
      getBuildingHistoryFormListWithPager(seachHistoryFormdata).then((res) => {
