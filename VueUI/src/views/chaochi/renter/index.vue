  ...
  <div class="app-container">
      ...
      <el-card>
        <el-form ref="searchform" :inline="true" :model="searchform" class="demo-form-inline" size="small">
          ...
          <el-form-item label="角色">
            <el-select v-model="searchform.RoleId" clearable placeholder="請選擇">
              <el-option v-for="item in selectRole" :key="item.Id" :label="item.FullName" :value="item.Id" />
            </el-select>
          </el-form-item>
          <el-form-item label="用戶資訊：">
            <el-input v-model="searchform.Keywords" clearable placeholder="帳號/姓名/暱稱/電話" />
          </el-form-item>
          <el-form-item label="註冊日期：">
            <el-date-picker v-model="searchform.CreateTime" type="daterange" align="right" value-format="yyyy-MM-dd" unlink-panels range-separator="至" start-placeholder="開始日期" end-placeholder="結束日期" :picker-options="pickerOptions" />
          </el-form-item>
          <el-form-item>
    ...
    <el-card>
      <div class="list-btn-container">
        <el-button-group>
          ...
          <el-button v-hasPermi="['User/Delete']" type="danger" icon="el-icon-delete" size="small" @click="deletePhysics()">刪除</el-button>
      ...
      >
        ...
        <el-table-column prop="Account" label="帳號" sortable="custom" width="230" fixed />
        <el-table-column prop="RealName" label="姓名" sortable="custom" width="180" fixed />
        <el-table-column prop="NickName" label="暱稱" sortable="custom" width="180" fixed />
        <el-table-column prop="Gender" label="性別" sortable="custom" width="90" align="center">
          <template slot-scope="scope">
            {{ scope.row.Gender=== 1 ? '男' : '女' }}
          </template>
        </el-table-column>
        <el-table-column prop="Birthday" label="生日" sortable="custom" width="120" align="center" :formatter="dateformatter" />
        <el-table-column prop="MobilePhone" label="手機號碼" sortable="custom" width="120" align="center" />
        <el-table-column prop="DepartmentName" label="公司/單位" width="260" align="center">
          <template slot-scope="scope">
            {{ scope.row.OrganizeName+"/"+ scope.row.DepartmentName }}
          </template>
        </el-table-column>
        <el-table-column prop="RoleName" label="角色" sortable="custom" width="280" align="center" />
        <el-table-column label="是否啟用" sortable="custom" width="90" prop="EnabledMark" align="center">
          <template slot-scope="scope">
            <el-tag :type="scope.row.EnabledMark === true ? 'success' : 'info'" disable-transitions>{{ scope.row.EnabledMark===true?'啟用':'禁用' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="是否刪除" sortable="custom" width="90" prop="DeleteMark" align="center">
          <template slot-scope="scope">
            <el-tag :type="scope.row.DeleteMark === true ? 'danger' : 'success'" disable-transitions>{{ scope.row.DeleteMark===true?'已刪除':'否' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="CreatorTime" label="添加時間" width="180" sortable />
    ...
    <el-dialog ref="dialogEditForm" :title="editFormTitle" :visible.sync="dialogEditFormVisible" width="660px">
      <el-form ref="editFrom" :inline="true" :model="editFrom" :rules="rules" class="demo-form-inline">
        <el-form-item label="帳號" :label-width="formLabelWidth" prop="Account">
          <el-input v-model="editFrom.Account" placeholder="請輸入帳號" autocomplete="off" clearable />
        </el-form-item>
        <el-form-item label="姓名" :label-width="formLabelWidth" prop="RealName">
          <el-input v-model="editFrom.RealName" placeholder="請輸入姓名" autocomplete="off" clearable />
        </el-form-item>
        <el-form-item label="暱稱" :label-width="formLabelWidth" prop="NickName">
          <el-input v-model="editFrom.NickName" placeholder="請輸入暱稱" autocomplete="off" clearable />
        </el-form-item>
        ...
        <el-form-item label="手機號碼" :label-width="formLabelWidth" prop="MobilePhone">
          <el-input v-model="editFrom.MobilePhone" placeholder="請輸入手機號碼" autocomplete="off" clearable />
        </el-form-item>
        ...
        <el-form-item label="生日" :label-width="formLabelWidth" prop="Birthday">
          <el-date-picker v-model="editFrom.Birthday" type="date" placeholder="選擇日期" />
        </el-form-item>
        <el-form-item label="選項" :label-width="formLabelWidth" prop="">
          <el-checkbox v-model="editFrom.EnabledMark">啟用</el-checkbox>
          <el-checkbox v-model="editFrom.IsAdministrator">管理員</el-checkbox>
        </el-form-item>
        <el-form-item label="公司/單位" :label-width="formLabelWidth" prop="DepartmentId">
        ...
        <el-form-item label="角色" :label-width="formLabelWidth" prop="RoleId">
          <el-select v-model="editFrom.RoleId" style="width:500px" multiple clearable placeholder="請選擇">
            <el-option v-for="item in selectRole" :key="item.Id" :label="item.FullName" :value="item.Id" />
          </el-select>
        </el-form-item>
