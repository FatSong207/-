<template>
  <div class="login-container">
    ...
    >
      ...
      <!-- <img src="static/img/login-logo.png" width="220" height="40"> -->
      <!-- </div> -->
      ...
      >登錄</el-button>
      <el-button
        type="text" @click.native.prevent="handleForgetPass"
      >忘記密碼?</el-button>
      <!-- <div style="text-align:right;">
        沒有帳號？<router-link :to="{name:'register'}">點此註冊</router-link>
      </div> -->
      <div class="tips" />
    </el-form>
    <div
    ...
    >
      <div
        class="footerNodelf text-secondary"
        @click="tryRemoveToken"
      >
        <span>本軟體使用權屬於：{{ companyName }} SHA:{{ GIT_HASH }}</span>
      </div>
    ...
    loadToken() {
      getToken().then((response) => {
        ...
        getSysSetting().then((res) => {
          ...
          this.companyLogo = require('@/assets/images/login-logo.png');
    ...
    handleForgetPass() {
      this.$confirm('系統將寄送新密碼至信箱', '警告', {
        ...
        if (!this.loginForm.username) {
          ...
          ForgetPassword(this.loginForm.username).then((res) => {
            console.log(res);
          });
        }
      });
...
.login-container {
  height: 100%;
  width: 100%;
  background-image: url('~@/assets/images/20230213_登入介面-01.jpg');
  overflow: hidden;
